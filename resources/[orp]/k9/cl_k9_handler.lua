...
AddEventHandler("K9:Follow", function()
	if current_dog ~= nil then
		if following == false then
			...
			following = true
			Notification(tostring(dog_name .. " Come!")) -- Come
		else
			ClearPedTasks(current_dog)
			following = false
			Notification(tostring(dog_name .. " Stay!")) -- Stay
		end
	end
end)
	...
	if attacking == false then
		if IsEntityAPed(other_ped) then
			if not IsEntityDead(other_ped) then
				...
				attacking = true
				following = false
				Notification(tostring(dog_name .. " Attack!")) -- Attack
			end
		end
	else
		attacking = false
		...
		Notification(tostring(dog_name .. " Good Dog!")) -- Stopping Dog (Good Dog)
	end
end)
...
function laydown(entity)
	...
	if IsEntityPlayingAnim(entity, animdicstart, animnamestart, 3) then
		...
		animation_played = nil
		Notification(tostring(dog_name .. " Stand!"))
	else
		RequestAnimDict(animdicstart)
		while not HasAnimDictLoaded(animdicstart) do
			Citizen.Wait(100)
		...
		Notification(tostring(dog_name .. " Lay Down!"))
	end
	...
	while true do
		...
		if attacking == true then
			if IsPedDeadOrDying(other_ped_attacked, 1) then
				Notification("Realease Bite!") -- Release Bite
				TriggerEvent("K9:Attack")
			end
		end
	end
