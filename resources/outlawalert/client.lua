...
RegisterNetEvent('deathPlace')
AddEventHandler('deathPlace', function(mx, my, mz)
for i = 0, #PedModels do
        if not origin and IsPedModel(GetPlayerPed(-1),GetHashKey(PedModels[i])) then
            if PlayerDeathAlert then
                local transM = 250
                local deathBlip = AddBlipForCoord(mx, my, mz)
                SetBlipSprite(deathBlip,  58)
                SetBlipColour(deathBlip,  1)
                SetBlipAlpha(deathBlip,  transG)
                SetBlipAsShortRange(deathBlip,  1)
                while transM ~= 0 do
                    Wait(blipPlayerDeath * 4)
                    transM = transM - 1
                    SetBlipAlpha(deathBlip,  transM)
                    if transM == 0 then
                        SetBlipSprite(deathBlip,  2)
                        return end
                end   
            end
        end
